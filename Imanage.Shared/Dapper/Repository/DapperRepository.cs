...
namespace Imanage.Shared.Dapper.Repository
{
      ...
      where TEntity : class
    {
        ...
        public async Task<IEnumerable<DTO>> ExecuteStoredProcedure<DTO>(string sql, DynamicParameters parameters)
        {
            ...
            var result = await Connection.QueryAsync<DTO>(sql, parameters, commandType: CommandType.StoredProcedure, commandTimeout: commandTimeout);

            return result;
        }

        public async Task<IEnumerable<DTO>> ExecuteStoredqueryProcedure<DTO>(string sql, DynamicParameters parameters)
        {
        ...
        public virtual IEnumerable<TEntity> GetLById(object id)
        {
            var task = GetByLIdAsync(id);
            task.WaitSync();
            return task.Result;
        }

        public virtual async Task<IEnumerable<TEntity>> GetByLIdAsync(object id)
        {
            if (Connection.State != ConnectionState.Open)
            {
                Connection.Open();
            ...
            return await Connection.QueryAsync<TEntity>(findQuery, new { Id = id });
